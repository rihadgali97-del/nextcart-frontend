...
export default function CustomerMessages({
  convos, loading, activeChat, setActiveChat,
  chatMessages, chatInput, setChatInput,
  chatLoading, chatBottomRef,
  sendMessage, openChat, profile,
}) {
  return (
    <div style={{display:"flex",height:"calc(100vh - 130px)",background:"#fff",
        ...
        <div style={{padding:"16px 18px",borderBottom:`1px solid ${C.border}`}}>
          ...
          <div style={{position:"relative"}}>
            ...
            <input placeholder="Search conversations…"
              style={{width:"100%",padding:"7px 10px 7px 28px",border:`1px solid ${C.border}`,
                borderRadius:8,fontSize:12,outline:"none",boxSizing:"border-box",background:"#f9fafb"}}/>
          </div>
        </div>
                ...
                return (
                  <div key={otherId||i} onClick={()=>openChat(c)}
                    style={{display:"flex",alignItems:"center",gap:12,padding:"12px 16px",
                      ...
                      background:isActive?C.sidebar:"#fff",borderBottom:`1px solid ${C.border}`}}
                    ...
                    <div style={{flex:1,minWidth:0}}>
                      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:2}}>
                        <div style={{fontWeight:600,fontSize:13,color:isActive?"#fff":C.text,
                          overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{otherName}</div>
                        {unread>0&&(
                          <span style={{background:C.red,color:"#fff",fontSize:9,fontWeight:700,
                            minWidth:16,height:16,borderRadius:20,display:"flex",
      ...
      {!activeChat ? (
        ...
        <div style={{flex:1,display:"flex",flexDirection:"column",minWidth:0}}>
          ...
          <div style={{padding:"14px 20px",borderBottom:`1px solid ${C.border}`,
            display:"flex",alignItems:"center",gap:12,background:"#fafbfa"}}>
          ...
          <div style={{flex:1,overflowY:"auto",padding:"16px 20px",
            display:"flex",flexDirection:"column",gap:10,background:"#f8fafb"}}>
            {chatLoading
              ? <div style={{textAlign:"center",padding:"40px 0"}}><Spinner size={22}/></div>
              : chatMessages.length===0
              ? <div style={{textAlign:"center",padding:"40px 0"}}>
                  <div style={{fontSize:32,marginBottom:8}}>👋</div>
                  <div style={{fontSize:12,color:C.muted}}>Say hello to {activeChat.name}!</div>
                </div>
              : chatMessages.map((msg,i)=>{
                  const isOwn=msg.sender===profile?._id||msg.sender?._id===profile?._id||msg.senderId===profile?._id;
                  return (
                    <div key={msg._id||i} style={{display:"flex",
                      justifyContent:isOwn?"flex-end":"flex-start",alignItems:"flex-end",gap:8}}>
                      {!isOwn&&<Avatar name={activeChat.name} size={28} bg={avatarColor(activeChat.name)}/>}
                      <div style={{maxWidth:"68%"}}>
                        <div style={{padding:"10px 14px",
                          borderRadius:isOwn?"18px 18px 4px 18px":"18px 18px 18px 4px",
                          background:isOwn?C.sidebar:"#fff",color:isOwn?"#fff":C.text,
                          fontSize:13,lineHeight:1.5,boxShadow:"0 1px 4px rgba(0,0,0,.08)",
          ...
          <div style={{padding:"12px 16px",borderTop:`1px solid ${C.border}`,
            ...
            <input value={chatInput} onChange={e=>setChatInput(e.target.value)}
              ...
              style={{flex:1,padding:"10px 14px",border:`1px solid ${C.border}`,borderRadius:24,
                fontSize:13,outline:"none",background:"#f9fafb",transition:"border-color .15s"}}
            ...
            <button onClick={sendMessage} disabled={!chatInput.trim()}
              style={{width:40,height:40,borderRadius:"50%",border:"none",
                ...
                cursor:chatInput.trim()?"pointer":"not-allowed",fontSize:18,
                display:"flex",alignItems:"center",justifyContent:"center",
                transition:"all .2s",flexShrink:0}}>➤</button>
          </div>
        </div>
      )}
    </div>
