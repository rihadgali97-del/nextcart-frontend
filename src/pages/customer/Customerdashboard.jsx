...
import API, {
  ...
  reportReview,
  deleteReview,
} from "../../services/api"; // adjust to your actual path
...
export default function CustomerDashboard() {
  ...
  // ══════════════════════════════════════════════════════════════════════════
  // SECTION: NEARBY (Proximity Search embedded)
  // ══════════════════════════════════════════════════════════════════════════
  ...
  const [nearbyLocStatus,setNearbyLocStatus]= useState("idle"); // idle|requesting|granted|denied
  ...
  const runNearbySearch = useCallback(async (overrides={}) => {
    setNearbyLoading(true);
    setNearbyError("");
    try {
      const params = {
        q:        overrides.query      ?? nearbyQuery,
        category: overrides.category   ?? nearbyCategory,
        minPrice: overrides.minPrice   ?? nearbyMinPrice,
        maxPrice: overrides.maxPrice   ?? nearbyMaxPrice,
        radius:   overrides.radius     ?? nearbyRadius,
        page:     overrides.page       ?? nearbyPage,
        limit:    NEARBY_LIMIT,
      };
      const loc = overrides.location ?? nearbyLocation;
      if (loc) { params.lng = loc.lng; params.lat = loc.lat; }
      Object.keys(params).forEach(k => { if (!params[k] && params[k] !== 0) delete params[k]; });

      const { data } = await API.get("/search", { params });
      setNearbyProducts(data.data || []);
      setNearbyPagination(data.pagination || { total:0, pages:1 });
      setNearbyProxInfo(data.proximity || null);

      if (data.proximity?.enabled) {
        const map = {};
        (data.data||[]).forEach((p,i) => { map[p._id] = i; });
  ...
  }, [nearbyQuery, nearbyCategory, nearbyMinPrice, nearbyMaxPrice, nearbyRadius, nearbyPage, nearbyLocation]);

  // Auto-search on filter change
  useEffect(() => {
    if (section !== "nearby") return;
    clearTimeout(nearbySearchRef.current);
    nearbySearchRef.current = setTimeout(() => runNearbySearch(), 350);
    ...
    <div style={{ display:"flex", flexDirection:"column", gap:16 }}>
      ...
      <div style={{ background:`linear-gradient(120deg, ${C.sidebar} 0%, #1a4d38 100%)`,
        ...
        {nearbyLocStatus !== "granted" ? (
          <button onClick={requestNearbyLocation}
            disabled={nearbyLocStatus==="requesting"}
          ...
          <div style={{ display:"flex", alignItems:"center", gap:8 }}>
            <div style={{ fontSize:11, color:"rgba(255,255,255,.6)" }}>
              ✓ Location active · {nearbyRadius}km radius
            </div>
            <button onClick={()=>{ setNearbyLocation(null); setNearbyLocStatus("idle"); }}
      ...
      <Card>
        <div style={{ display:"flex", gap:10, flexWrap:"wrap", alignItems:"center" }}>
          ...
          <input type="number" placeholder="Min $" value={nearbyMinPrice}
            ...
            style={{ width:80, padding:"9px 10px", border:`1px solid ${C.border}`,
          ...
          {nearbyLocStatus==="granted" && (
            <div style={{ display:"flex", alignItems:"center", gap:8, marginLeft:"auto" }}>
              ...
              <span style={{ fontSize:12, fontWeight:700, color:C.dark,
                whiteSpace:"nowrap" }}>{nearbyRadius}km</span>
            </div>
          )}
        </div>
      </Card>
            ...
            : <span>
                ...
                {nearbyProxInfo?.enabled && (
                  <span style={{ color:C.green, marginLeft:6 }}>
                    · 📍 within {nearbyProxInfo.radiusKm}km of you
                  </span>
                )}
              </span>
          }
      ...
      {nearbyError && (
        <div style={{ padding:"12px 16px", background:`${C.red}10`,
          border:`1px solid ${C.red}30`, borderRadius:9, color:C.red, fontSize:13 }}>
          ⚠ {nearbyError}
        </div>
      )}
      ...
      {!nearbyLoading && nearbyProducts.length === 0 ? (
        <Card style={{ textAlign:"center", padding:"60px 20px" }}>
          ...
          {nearbyLocStatus==="granted" && (
            <Btn onClick={()=>{ setNearbyRadius(100); setNearbyPage(1); }}>
              Expand to 100km
            </Btn>
          )}
          {nearbyLocStatus!=="granted" && (
        ...
        <div style={{ display:"grid",
          ...
          {nearbyProducts.map((p, i) => {
            const rank = nearbyLocStatus==="granted"
              ? (nearbyRankMap[p._id] ?? i) : undefined;
            return (
                ...
                <div style={{ padding:"12px 14px" }}>
                    ...
                    <div style={{ marginBottom:8 }}>
                      <div style={{ display:"flex", justifyContent:"space-between",
                        ...
                        <span style={{ fontWeight:700, color:C.dark }}>
                          {Math.round(p.vendor.trustScore)}/100
                        </span>
                      </div>
                      <div style={{ height:3, background:"#e8ede9",
                  ...
                  <div style={{ display:"flex", alignItems:"center",
                    ...
                    <button onClick={e=>{ e.stopPropagation(); handleAddToCart(p._id,e); }}
                      disabled={p.stock===0}
                      style={{ background:p.stock===0?"#ccc":C.sidebar,
                        color:p.stock===0?"#999":C.gold, border:"none",
      ...
      {nearbyPagination.pages > 1 && (
        <Pagination
          page={nearbyPage}
          totalItems={nearbyPagination.total}
          perPage={NEARBY_LIMIT}
          onChange={p=>{ setNearbyPage(p); window.scrollTo(0,0); }}
        />
      )}
