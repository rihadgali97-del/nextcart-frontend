...
export const Pill = ({ label }) => {
  ...
  return (
    <span style={{ background:s.bg, color:s.color, padding:"3px 10px", borderRadius:20,
      ...
      gap:4, textTransform:"capitalize" }}>
      {s.icon} {label||"—"}
    </span>
  );
};
    ...
    {[1,2,3,4,5].map(i=>(
      <span key={i} onClick={()=>interactive&&onChange?.(i)}
        style={{ fontSize:size, color:i<=rating?C.gold:"#d0d5d1",
          cursor:interactive?"pointer":"default" }}>★</span>
    ))}
  </span>
);
...
export const Panel = ({ title, subtitle, action, onAction, children, style={} }) => (
  <Card style={style}>
    <div style={{ display:"flex", alignItems:"flex-start", justifyContent:"space-between", marginBottom:16 }}>
      ...
      {action&&<button onClick={onAction} style={{ fontSize:12, color:C.green, background:"none",
  ...
  return (
    <button type={type} onClick={onClick} disabled={disabled}
      style={{ ...v, padding:"9px 20px", borderRadius:8, fontSize:13, fontWeight:600,
        cursor:disabled?"not-allowed":"pointer", opacity:disabled?.6:1, ...style }}
...
export const Pagination = ({ page, totalItems, perPage=ITEMS_PER_PAGE, onChange }) => {
  ...
  const start=(page-1)*perPage+1, end=Math.min(page*perPage,totalItems);
  return (
      ...
      <div style={{ display:"flex", gap:6 }}>
        <button disabled={page<=1} onClick={()=>onChange(page-1)}
          style={{ padding:"6px 14px", fontSize:12, fontWeight:600, borderRadius:7,
            ...
            color:page<=1?C.muted:C.text, cursor:page<=1?"not-allowed":"pointer" }}>← Prev</button>
        {Array.from({length:totalPages},(_,i)=>i+1)
          .filter(n=>n===1||n===totalPages||Math.abs(n-page)<=1)
          .reduce((acc,n,i,arr)=>{ if(i>0&&n-arr[i-1]>1)acc.push("..."); acc.push(n); return acc; },[])
          .map((n,i)=>n==="..."
            ?<span key={`d${i}`} style={{ fontSize:12,color:C.muted,padding:"0 2px" }}>…</span>
            :<button key={n} onClick={()=>onChange(n)}
              style={{ width:32,height:32,borderRadius:7,fontSize:12,fontWeight:600,
                ...
                color:n===page?C.gold:C.text,cursor:"pointer" }}>{n}</button>
          )}
        <button disabled={page>=totalPages} onClick={()=>onChange(page+1)}
          style={{ padding:"6px 14px",fontSize:12,fontWeight:600,borderRadius:7,
            border:`1px solid ${C.border}`,background:page>=totalPages?"#f3f5f1":"#fff",
            color:page>=totalPages?C.muted:C.text,cursor:page>=totalPages?"not-allowed":"pointer" }}>Next →</button>
      </div>
    </div>
  );
};
...
export const Donut = ({ slices=[], size=110, label="" }) => {
  ...
  let cur=-90;
  const r=42,cx=size/2,cy=size/2,rad=d=>d*Math.PI/180;
      ...
      {data.map((d,i)=>{
        ...
        return (
          <g key={i}>
            ...
            <text x={i*(bW+gap)+bW/2} y={height+14} textAnchor="middle" fontSize={8} fill={C.muted}>{d.label}</text>
          </g>
        );
      })}
    </svg>
